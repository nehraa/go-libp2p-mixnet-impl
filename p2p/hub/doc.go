...
// receptors on the same listening port. The hub refuses to start if the host
// already owns the configured protocol handler, and callers can choose whether
// event-buffer overflow resets the active stream or only drops the event.
package hub
